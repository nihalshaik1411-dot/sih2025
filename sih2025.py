...
def draw_scene(moving_blocks=None, note=""):
    ...
    for i in range(st.session_state.blocks_top_A):
        y0 = 50 + i * 2.1
        fig.add_shape(type="rect", x0=-2.4, x1=-1.2, y0=y0, y1=y0 + 1.9, fillcolor="#2b6cb0", line=dict(color="black"))
    # Draw stacked blocks at top B (right, red)
    for i in range(st.session_state.blocks_top_B):
        y0 = 50 + i * 2.1
        fig.add_shape(type="rect", x0=1.2, x1=2.4, y0=y0, y1=y0 + 1.9, fillcolor="#c53030", line=dict(color="black"))

    # Tied block at bottom C (left, gray if present)
    if st.session_state.tied_bottom_C > 0:
        fig.add_shape(type="rect", x0=-2.4, x1=-1.2, y0=-51, y1=-49.1, fillcolor="gray", line=dict(color="black"))
    # Tied block at bottom D (right, gray if present)
    if st.session_state.tied_bottom_D > 0:
        fig.add_shape(type="rect", x0=1.2, x1=2.4, y0=-51, y1=-49.1, fillcolor="gray", line=dict(color="black"))

    # Stored blocks at left (below tied, orange)
    num_stored_left = st.session_state.storage_left // 10
    base_y_left = -51.1
    for i in range(num_stored_left):
        y1 = base_y_left - i * 2.1
        y0 = y1 - 1.9
    ...
    if moving_blocks:
        for pt, color, y, size_kg, label, block_index in moving_blocks:
            ...
            if pt == "left":
                ...
                x0, x1 = -0.8, 0.8
                y_offset = y
            fig.add_shape(type="rect", x0=x0, x1=x1, y0=y_offset, y1=y_offset + 1.9, fillcolor=color, line=dict(color="black"))
            fig.add_annotation(x=(x0 + x1) / 2, y=y_offset + 2.2, text=f"{label}: {size_kg}kg", showarrow=False)
